...
def start_plotting(fig_size, fig_pos, style="ticks", rc=None, despine=False):
...
def ax_plot_lines(ax, xs, ys, colors, shapes, linestyles,
        errorbar=False, linewidth=LINEWIDTH, markersize=20):
    lines = []
...
def plot_lines(xs, ys, title=None, xlabel=None,
               ...
               ticksize=None, style="whitegrid", markersize=20,
               filename=None, legend_mode=None, legend_cols=1):
    ...
    if legend:
        ax.legend(lines, legend, loc=loc, bbox_to_anchor=bbox_to_anchor,
            frameon=False, mode=legend_mode, ncol=legend_cols)
    if not xlim:
        max_x = np.max(np.max(xs))
        min_x = np.min(np.min(xs))
        diff = max_x - min_x
        xlim = (min_x - 0.02 * diff, max_x + 0.02 * diff)
...
def plot_dict_array(dict_list, title=None, xlabel=None,
        ...
        label_text=None, style="whitegrid", legend_mode=None,
                   legend_cols=1, markersize=20):
    ...
    return plot_lines(xs, ys, title=title, xlabel=xlabel,
        ...
        label_text=label_text, style=style, legend_mode=legend_mode,
                     legend_cols=legend_cols, markersize=markersize)
...
def joint_plot(x, y, xlabel=None,
    ...
    with sns.axes_style("darkgrid"):
        ...
        g.plot_sub_joint(plt.scatter, 500, s=20, c=scatter_color, alpha=0.3)
        ...
        # g = sns.jointplot(x, y, size=size,
        #         annot_kws=dict(fontsize=25, template="{stat} = {val:.2g}\np = {p:.2g}"),
        #         space=0.1, ratio=2, joint_kws=dict(s=markersize))
        # g.plot_joint(sns.kdeplot, zorder=0, n_levels=6)
    return g

def plot_power_law_fit(data, xlabel=None,
                       fig_size=FIG_SIZE, fig_pos=None):
    fig, ax = start_plotting(fig_size, fig_pos)
    fit = powerlaw.Fit(data, discrete=True, xmin=1)
    fit.plot_ccdf(color='#cf6275', ax=ax, label="Empirical, xmin=1")
    # fit.power_law.plot_ccdf(color='#cf6275', linestyle='--', ax=ax, label="Fit, xmin=1")
    fit = powerlaw.Fit(data, discrete=True)
    xmin = fit.power_law.xmin
    alpha = fit.power_law.alpha
    fit.plot_ccdf(color='#0485d1', ax=ax, label="Empirical, xmin=%d" % xmin)
    fit.power_law.plot_ccdf(color='#0485d1', linestyle='--', ax=ax, label="Fit, xmin=%d, $\\alpha$=%.3f" % (xmin, alpha))
    ...
    if xlabel:
        ax.set_xlabel(xlabel)
    # end_plotting(fig, ax, xlabel=xlabel,
    #              ylabel="CCDF")
    return fig
